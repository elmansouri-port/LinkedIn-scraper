...
from api.dependencies import create_job, get_job, update_job, append_job_log
...
def _run_with_driver(job_id: str, task_fn, require_auth: bool = True):
...
def run_google_scrape_task(job_id: str, request: GoogleScrapeRequest):
    """Background task for Google-based scraping. No LinkedIn auth needed."""
    def task(driver, jid):
        def on_progress(pct: int):
            update_job(jid, progress=pct)

        def on_log(level: str, message: str):
            append_job_log(jid, level, message)
    ...
    _run_with_driver(job_id, task, require_auth=False)
