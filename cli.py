...
def _ensure_authenticated(driver, auth_manager):
    ...
    When a Chrome profile was previously selected by the user (via "Select
    Chrome profile" in the auth menu), this function skips all login prompts
    and trusts the profile to supply the authenticated session.

    It navigates to LinkedIn feed to verify — if the URL redirects to a login
    page, the session has expired, and it falls through to:
        saved cookies → credentials → manual login.

    Without a saved profile, the original auto-login flow runs:
        verify → cookies → credentials → manual.
    ...
    # ── Saved profile path — skip login prompts, trust the profile ──
    active_profile = DriverManager.get_active_profile_config()
    if active_profile:
        logger = get_logger("cli.auth")
        logger.info("Saved profile '%s' active — checking session", active_profile.get("name", "?"))
        try:
            ...
            if "login" in driver.current_url.lower():
                raise ValueError("redirected_to_login")
        except (TimeoutException, WebDriverException):
            logger.warning("Feed page timed out — continuing anyway (scraper handles login redirects at runtime)")
            driver.set_page_load_timeout(30)
            return True, auth_manager
        except ValueError:
    ...
    # ── Fresh profile — original auto-login flow ──
